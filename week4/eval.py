...
def EQUAL_3(i, j):
    ...
    notxor1 = NOT(xor1)
    notxor2 = NOT(xor2)
    and1 = AND(notxor0, notxor1)
    return AND(and1, notxor2)
...
def EVAL_3_7_4_1(program, input_bits):
    ...
    T = UPDATE_7(T, input_bits[0], '000')
    T = UPDATE_7(T, input_bits[1], '001')
    T = UPDATE_7(T, input_bits[2], '010')
    a = GET_7(T, program[6 : 9])
    b = GET_7(T, program[3 : 6])
    ans = NAND(a, b)
    T = UPDATE_7(T, ans, program[0 : 3])
    a = GET_7(T, program[15: 18])
    b = GET_7(T, program[12: 15])
    ans = NAND(a, b)
    T = UPDATE_7(T, ans, program[9: 12])
    a = GET_7(T, program[24: 27])
    b = GET_7(T, program[21: 24])
    ans = NAND(a, b)
    T = UPDATE_7(T, ans, program[18: 21])
    a = GET_7(T, program[33: 36])
    b = GET_7(T, program[30: 33])
    ans = NAND(a, b)
    T = UPDATE_7(T, ans, program[27: 30])
    return T[6]
...
or_program = [(3,0,0),(4,1,1),(6,3,4),(0,0,0)]
or_program = prog2bits(or_program, 3)
